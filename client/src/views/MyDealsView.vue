        ...
        <article v-for="deal in deals" :key="deal.id" class="deal-row">
          ...
          <div class="deal-row-info">
            ...
            <div class="deal-row-meta">
              <span :class="deal.verified ? 'verified' : 'unverified'">{{ deal.verified ? 'Verified' : 'Pending review' }}</span>
...
<style scoped>
.my-deals-page { padding: 40px 0 60px; animation: fade-in 0.4s ease; }
...
.skeleton-row { height: 112px; border-radius: var(--radius-md); }
.deal-row { display: grid; grid-template-columns: 96px 1fr auto; align-items: center; gap: 16px; padding: 16px; background: var(--color-card-bg); border: 1px solid var(--color-border); border-radius: var(--radius-md); }
.deal-row img { width: 96px; height: 76px; border-radius: var(--radius-sm); object-fit: cover; }
.deal-row-info h3 { font-size: 1rem; font-weight: 700; margin-bottom: 8px; color: var(--color-text); }
.deal-row-meta { display: flex; flex-wrap: wrap; gap: 14px; font-size: 0.86rem; color: var(--color-text-secondary); }
.deal-row-actions { display: flex; gap: 8px; flex-wrap: wrap; justify-content: flex-end; }
.verified { color: var(--color-success); font-weight: 700; }
.unverified { color: var(--color-warning); font-weight: 700; }
.danger { color: var(--color-error); }
@media (max-width: 760px) { .deal-row { grid-template-columns: 1fr; } .deal-row img { width: 100%; height: 180px; } .deal-row-actions { justify-content: stretch; } .deal-row-actions .btn { flex: 1; } }
</style>
