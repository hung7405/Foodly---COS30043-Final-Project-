  ...
  <div class="merchant-page">
    <div class="container">
            ...
            <div v-if="dashboard?.recentOrders?.length" class="order-list">
              <div v-for="o in dashboard.recentOrders" :key="o.id" class="order-row">
                ...
                <div class="order-info">
                  ...
                  <span class="order-meta">{{ formatDate(o.reservedAt) }} · {{ o.quantity }} × · {{ o.reservationCode }}</span>
                </div>
      ...
      <div v-else-if="!error" class="empty-state">
        ...
        <p>This account does not own any stores yet. Assign <code>stores.user_id</code> to link a store to this merchant.</p>
      </div>
    </div>
  </div>
</template>
...
.metric-value { font-size: 1.6rem; font-weight: 800; color: var(--color-text); margin-bottom: 4px; }
.metric-label { font-size: 0.8125rem; color: var(--color-text-secondary); }

.dashboard-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
.panel { padding: 22px; background: var(--color-card-bg); border: 1px solid var(--color-border); border-radius: var(--radius-lg); }
...
.bar { width: 100%; max-width: 46px; background: var(--color-accent); border-radius: 4px 4px 0 0; transition: height 0.5s ease; }
.bar-value { font-size: 0.68rem; color: var(--color-text-secondary); font-weight: 600; white-space: nowrap; }
.bar-label { font-size: 0.72rem; color: var(--color-text-tertiary); margin-top: 4px; }

.product-list, .alert-list, .order-list { display: flex; flex-direction: column; }
.product-row, .alert-row, .order-row { display: flex; align-items: center; gap: 12px; padding: 10px 0; border-bottom: 1px solid var(--color-border); font-size: 0.875rem; }
.product-row:last-child, .alert-row:last-child, .order-row:last-child { border-bottom: none; }
.product-rank { width: 22px; height: 22px; border-radius: 50%; background: var(--color-bg-tertiary); display: flex; align-items: center; justify-content: center; font-size: 0.75rem; font-weight: 700; color: var(--color-text-secondary); flex-shrink: 0; }
.product-name { flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.product-sold { color: var(--color-accent); font-weight: 700; flex-shrink: 0; }

.alert-dot { width: 8px; height: 8px; border-radius: 50%; background: #f59e0b; flex-shrink: 0; }
.alert-dot.critical { background: var(--color-error); }
.alert-title { flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.alert-count { color: var(--color-text-secondary); font-weight: 600; flex-shrink: 0; }

.status-badge { padding: 4px 10px; border-radius: 999px; font-size: 0.72rem; font-weight: 800; text-transform: uppercase; background: var(--color-bg-tertiary); flex-shrink: 0; }
.status-badge.active { background: #ecfdf5; color: #166534; }
.status-badge.confirmed { background: #dcfce7; color: #166534; }
.status-badge.cancelled, .status-badge.expired { background: #f1f5f9; color: #64748b; }

.order-info { flex: 1; display: flex; flex-direction: column; min-width: 0; }
.order-title { font-weight: 600; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.order-meta { font-size: 0.75rem; color: var(--color-text-tertiary); }
.order-amount { font-weight: 700; flex-shrink: 0; }

.empty-inline { padding: 20px 0; color: var(--color-text-tertiary); font-size: 0.875rem; text-align: center; }
...
@media (max-width: 768px) { .metrics-grid { grid-template-columns: 1fr 1fr; } .dashboard-grid { grid-template-columns: 1fr; } }
</style>
